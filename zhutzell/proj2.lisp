...
(defun learn (&key (k 1)
  ...
  (let ((training (load train))
        (testing (load test))
        (clusters nil))
    (setf training (apply prep (list training)))
    (setf testing (apply prep (list testing)))
    (setf training (apply discretizer (list training)))
    (setf clusters (apply cluster (list training)))))
