...
(defun stable-theory (tbls &key (k 1)
  ...
  (let ((test-tbls nil)
    ...
    (format t "number of slices = ~a ~%" (length test-tbls))
    ...
    (dolist (test (shuffle test-tbls))
      (let ((new-g 0)
    	...
    	(statistics-output cv-results :file-name file-name)  ; "append" to output file
        ...
        (if (< (length (table-all train)) 1000)
          (if (or (< new-g 0.60) (> (- prev-g new-g) 0.1))
  	    (progn 
	      (setf train (combine-preprocessor train test))
	      (format t "plus one, train size ~a ~,2F ~,1F ~%" (length (table-all train)) new-g (- prev-g new-g)))))
        (setf prev-g new-g)
        (push result results)))
    (setf results (nreverse results))
    (statistics-output (filter #'(lambda (result) (and (equalp (statistics-class result) defect-class) result)) results))))

(defun stable-theory-demo ()
	(stable-theory (list (ar3com)(ar4com)(ar5com)(pc1com)(kc1com)(kc2com)(cm1com)) :prep #'numeric-preprocessor :discretizer #'10bins-eq-freq :classifier-train #'nb-train :classifier #'nb-classify :file-name "stable4x25x15-ht9-1000.txt"))
