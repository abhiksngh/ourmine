    ...
    (let* ((trainSet (if (tablep per-data-train)
      ...
      (multiple-value-bind (trainSet testSet)
        ...
        (multiple-value-bind (trainSet testSet)
          ...
          (doitems (per-test j (table-all testSet))
            (let* ((resultSet)
                   (nearestTrainList (dolist (each-item (k-nearest-per-instance per-test trainSet) (reverse resultSet))
                                   (push (eg-features(nth (first each-item) (table-all trainSet))) resultSet )))
                   (nearestTrain (subseq nearestTrainList 0 (if (< (length nearestTrainList) k)
                                                                (length nearestTrainList)
                                                                k)))
  ...
  (let* ((xtrain (xindex train))
    ...
    (doitems (per-instance i xtest returnData)
      (let* ((nearest-neighbors (k-nearest-per-instance per-instance xtrain))
             (nearest-neighbors (if (< (length nearest-neighbors) 5)
                                    nearest-neighbors
                                    (subseq nearest-neighbors 0 5))))
