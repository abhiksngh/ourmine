...
(defun test-all (train &optional (times 10) (k 1) (n 5))
  (let* (disc-nb disc-info disc-info-nb)
    ...
    (write-stats (concatenate 'string "no-disc-nb-cluster-" (format nil "~a"  k) "-" (format nil "~a"  n))  (format nil "~a" disc-nb))  
    (setf disc-info (test-disc-infogain-centroid-nb train n times))
    (write-stats (concatenate 'string "disc-info-cluster-" (format nil "~a"  k) "-" (format nil  "~a" n))(format nil "~a" disc-info))
    (setf disc-info-nb (test-disc-infogain-nb train times n))
    (write-stats (concatenate 'string "disc-info-nb-" (format nil "~a"  k) "-" (format nil "~a"  n)) (format nil "~a" disc-info-nb))))
