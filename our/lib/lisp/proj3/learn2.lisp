...
(defun test-all-no-subsample (&optional (times 10))
  (let* ((data-shared '(shared_pc1 shared_cm1 shared_kc1 shared_kc2 shared_kc3 shared_mc2 shared_mw1))
    ...
    (dolist (ar data-ar)
      (setf ar-joined (append ar-joined (list (funcall ar)))))
    (setf ar-joined-table (xindex (append-tables ar-joined)))
    (format t "~a~%" "Turkish dataset combined")
    (setf results-ar (append results-ar (test-no-subsample ar-joined-table times)))
...
(defun test-no-subsample (train &optional (times 10))
  (let* ((preprocess)
    ...
    (format t "~a~a~%" "Preprocessed data knn " (length (table-all preprocess)))
