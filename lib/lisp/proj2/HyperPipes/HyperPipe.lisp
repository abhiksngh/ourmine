...
(defun demoHyperPipesNew(&optional (dataFileName "primary-tumor") (Alpha 0) (countType 0) (meanType 0))
  (print "*************Demoing HyperPipes**************")
  ...
  (let ((MyHyperPipes (list))
        (totalChecks 0)
        (totalRight 0)
        (outputFile (open (concatenate `string "outputFile" (format nil "~a" countType) (format nil "~a" meanType) ".txt") :direction :output :if-does-not-exist :create :if-exists :overwrite)))
    (with-open-file (stream (concatenate `string "proj2/HyperPipes/Data/" dataFileName ".lisp"))
      (do ((line (read-line stream nil) (read-line stream nil))) ((null line))
        (let* ((attributeValues (eval (read-from-string line)))
               (tiedClasses (GetTiedClasses2 MyHyperPipes attributeValues Alpha countType meanType))
...
(defun GetTiedClasses2(MyHyperPipes AttributeValues Alpha countType meanType)
  (let* ((currentMax most-negative-fixnum)
         (HighestValue ())
