...
public class Main {
    ...
    public void comParse(String args[])
    {
        ...
        }else if(clusterer.equalsIgnoreCase("-c"))
        {
            ...
            //output has to be handled differently, as there exists a list of cluster groups
            Output out = new Output();

            int currentClusterGroup=0;

            for(ArrayList<Cluster> clusterGroup : canopy.getFinalClusterGroups())
            {
                out.outToSparseArff(_instances, _attributes, clusterGroup,
                        "clustergroup="+currentClusterGroup+"_"+outfile);
                currentClusterGroup++;
            }
